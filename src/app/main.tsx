...
import { mockRidesRecent } from '../mock/data';
...
export default function MainScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem('user')
      .then(raw => { if (raw) setUserName(JSON.parse(raw).name); })
      .catch(() => {});
  }, []);

  const rides = mockRidesRecent.data;
    ...
    <View style={{ flex: 1, backgroundColor: T.bgAlt }}>
      ...
      <ScrollView contentContainerStyle={s.content}>
        ...
        <TouchableOpacity style={s.ctaCard} onPress={() => router.push('/qr-scan')} activeOpacity={0.85}>
          <View style={s.ctaTop}>
            ...
            <View style={s.ctaPill}>
              <Text style={s.ctaPillText}>라이딩 없음</Text>
            </View>
          </View>
          <Text style={s.ctaSub}>스쿠터 QR을 스캔하여 라이딩을 시작하세요</Text>
          <View style={s.ctaBtn}>
            <Text style={s.ctaBtnText}>📷  QR 스캔으로 시작</Text>
          </View>
        </TouchableOpacity>
        ...
        <View>
          ...
          {/*
            핵심: flexDirection row + flexWrap wrap
            각 버튼에 flexBasis '50%' + padding으로 gap 구현
            → 항상 2열, 양옆이 컨테이너에 꽉 참
          */}
        ...
        <View>
          ...
          {rides.length === 0 ? (
            <WFCard>
              <Text style={s.emptyText}>이용 기록이 없습니다</Text>
            </WFCard>
          ) : (
            rides.map(r => (
              <WFCard key={r.ride_id} style={s.rideCard}>
                <View>
                  ...
                  <Text style={s.rideDate}>
                    {fmt(r.started_at)} · {dur(r.started_at, r.ended_at)}
                  </Text>
                </View>
                {r.warning_count > 0
...
const s = StyleSheet.create({
  ...
  ctaBtn: {
    height: 44, backgroundColor: 'rgba(255,255,255,0.15)',
  ...
  quickGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    // 카드 사이 gap을 패딩으로 구현 (gap prop은 웹에서 불안정)
